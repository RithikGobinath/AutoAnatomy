...
from autoanatomy.engine.weights import nostdout
...
from autoanatomy.engine.custom_trainers import (nnUNetTrainer_MOSAIC_1k_QuarterLR_NoMirroring,
                                              nnUNetTrainerDiceTopK10Loss_2000epochs,
                                              nnUNetTrainerSkeletonRecall)
from autoanatomy.engine.nnunet_runtime_patches import patch_nnunet_cropped_logits_resampling
...
from autoanatomy.engine.class_map import class_map, class_map_5_parts, class_map_5_parts_total_v3, class_map_parts_mr, class_map_parts_headneck_muscles
from autoanatomy.engine.class_map import map_taskid_to_partname_mr, map_taskid_to_partname_ct, map_taskid_to_partname_ct_v3, map_taskid_to_partname_headneck_muscles
from autoanatomy.engine.alignment import as_closest_canonical_nifti, undo_canonical_nifti
from autoanatomy.engine.alignment import as_closest_canonical, undo_canonical
from autoanatomy.engine.resampling import change_spacing
from autoanatomy.engine.weights import combine_masks, compress_nifti, check_if_shape_and_affine_identical, reorder_multilabel_like_v1
from autoanatomy.engine.dicom_io import dcm_to_nifti, save_mask_as_rtstruct, save_mask_as_dicomseg
from autoanatomy.engine.cropping import crop_to_mask_nifti, undo_crop_nifti
from autoanatomy.engine.cropping import crop_to_mask, undo_crop
from autoanatomy.engine.postprocessing import remove_outside_of_mask, extract_skin, remove_auxiliary_labels
from autoanatomy.engine.postprocessing import keep_largest_blob_multilabel, remove_small_blobs_multilabel
from autoanatomy.engine.postprocessing import postprocess_vertebrae_pp
from autoanatomy.engine.postprocessing import refine_vertebrae_pp_with_body_mask
from autoanatomy.engine.nifti_ext_header import save_multilabel_nifti, add_label_map_to_nifti
from autoanatomy.engine.statistics import get_basic_statistics
...
def nnUNet_predict_image(file_in: Union[str, Path, Nifti1Image], file_out, task_id, model="3d_fullres", folds=None,
    ...
    # ignore_cleanup_errors: on Windows a worker process (nnU-Net's background
    # export thread, or an antivirus scan) can still hold a handle on a file
    # inside tmp_folder for a moment after we're done with it, which makes the
    # default cleanup raise PermissionError (WinError 32). The run itself has
    # already succeeded by this point, so don't fail the whole segmentation
    # over a temp-dir cleanup race -- just leave the (now-empty-ish) dir behind.
    with tempfile.TemporaryDirectory(prefix="nnunet_tmp_", ignore_cleanup_errors=True) as tmp_folder:
